...
from tools.music_control import trigger_mpd_library_update # Added for MPD library update
...
async def initialize_global_engines(app_settings: Settings):
    ...
    **Guidelines:**
    ...
    manager = ConnectionManager(
        ...
        sound_device_settings=app_settings.sounddevice
    )
...
class ConnectionManager:
    def __init__(self, wake_word_engine: Optional[WakeWordEngineBase],
                 ...
                 vad_processing_settings: "settings.VADSettings",
                 sound_device_settings: SoundDeviceSettings): # type: ignore
            ...
            if self.global_audio_settings.play_activation_sound and self.global_audio_settings.activation_sound_path:
                ...
                if sound_path and os.path.exists(sound_path):
                    with wave.open(sound_path, 'rb') as wf:
                        ...
                        self.activation_sound_sample_rate = wf.getframerate() # Сохраняем исходную SR
                        ...
                        logger.info(f"Activation sound loaded: {sound_path}, SR={self.activation_sound_sample_rate}Hz, "
                                    f"Channels={self.activation_sound_channels}, Width={self.activation_sound_sampwidth}bytes, "
                                    f"Size={len(self.activation_sound_bytes)} bytes")

                        # Ресемплинг звука активации, если включен fixed_output_sample_rate и частоты не совпадают
                        ...
                        if target_sr_for_output is not None and \
                           ...
                           self.activation_sound_bytes:
                            if SCIPY_AVAILABLE:
                                logger.info(f"Resampling activation sound from {self.activation_sound_sample_rate}Hz "
                                            f"to {target_sr_for_output}Hz.")
                                self.activation_sound_bytes = resample_audio_bytes(
                                    ...
                                    dtype_str=f'int{self.activation_sound_sampwidth*8}' # e.g. 'int16'
                                )
                                # После ресемплинга, его "эффективная" частота для воспроизведения - это target_sr_for_output
                                # Но для play_tts_bytes мы все равно передадим target_sr_for_output как sample_rate,
                                # так что SoundDeviceOutputEngine не будет пытаться его снова ресемплить, если он уже нужной частоты.
                                # Для ясности, можно обновить self.activation_sound_sample_rate,
                                # но важнее, что play_tts_bytes получит правильный SR.
                                # self.activation_sound_sample_rate = target_sr_for_output # Можно и так для консистентности
                            else:
                                logger.warning("SciPy not available, cannot resample activation sound. "
                                             "Playback might be at incorrect speed or fail if sample rates differ significantly from output stream.")
                        
                        # Проверки на каналы и битность после потенциального ресемплинга (хотя ресемплинг не меняет эти параметры)
                        if self.activation_sound_channels != 1 and self.sd_settings.fixed_output_sample_rate is None:
                             # Если не фиксированный режим, и звук не моно, могут быть проблемы с некоторыми TTS, которые ожидают моно.
                             # Sounddevice сам разберется с каналами при воспроизведении.
                             logger.debug(f"Activation sound has {self.activation_sound_channels} channels.")
                        if self.activation_sound_sampwidth != 2:
                             logger.warning(f"Activation sound '{sound_path}' sample width is {self.activation_sound_sampwidth} bytes "
                                          f"(expected 2 for int16). Playback might be incorrect if not handled by output engine.")
        except Exception as e:
            ...
            if keyword_index >= 0:
                ...
                if self.global_audio_settings.play_activation_sound:
                    if self.is_websocket_active:
                        ...
                        # Определяем, с какой SR был загружен (и, возможно, ресемплирован) звук активации
                        # Если fixed_output_sample_rate был применен, то звук уже на этой частоте.
                        # Если нет, то на исходной.
                        ...
                        if sr_for_activation_playback is None: # Если self.activation_sound_sample_rate тоже None (не загрузился)
                            logger.warning("Activation sound sample rate unknown, cannot play.")
                        else:
                            logger.info(f"Playing activation sound locally (effective SR for playback: {sr_for_activation_playback}Hz).")
                            self.local_audio_output_engine.play_tts_bytes(
                                self.activation_sound_bytes,
                                sample_rate=sr_for_activation_playback # Передаем частоту, на которой звук ПОДГОТОВЛЕН
                            )
